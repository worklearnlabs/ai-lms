...
export async function middleware(request: NextRequest) {
  // First, update the auth session
  const response = await updateSession(request);
  
  // Get a Supabase client with the cookies from the request
  const supabase = createServerClient(
    ...
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
    {
      cookies: {
        ...
        set: () => {}, // We don't need to set or remove in this part of middleware
        remove: () => {}, // as we already did that in updateSession
      },
    }
  );
